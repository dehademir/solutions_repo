...
1. The algorithm must **detect**, **analyze**, and **automatically simplify** arbitrary resistor networks.
2. The algorithm must also produce **clear visualizations** of each step in the simplification process.
...
2. **After Parallel Reduction**
   ...
   - Equivalent parallel resistance:
     $$
     R_{BD} = \left(\frac{1}{6} + \frac{1}{4}\right)^{-1} = 2.4\,\Omega
     $$
   - **Remaining resistors:**
     - A–B = 2 Ω
     - B–D = 2.4 Ω
...
- **Reduction Rules:**
  1. **Parallel:** $R_\text{eq}=\left(\sum \frac{1}{R_i}\right)^{-1}$
  2. **Series:** $R_\text{eq}=\sum R_i$
